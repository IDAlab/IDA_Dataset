...
class flowGenerator:
    ...
    def __init__(self, pcap_path, ip):
        ...
        '''
            initializer
            set pcap file path
            set outgoing ip address
        '''
        self._pcap_path = pcap_path
        self._src_ip = ip
        self.total_cell, self.total_time_list, self.out_time_list, self.in_time_list = self.readPcap(self._pcap_path)
    ...
    def readPcap(self, pcap_path):
        ...
        try:
            for timestamp, buf in pcap:
                ...
                # Convert TCP layer to Tor cell layer
                # Also, in this script, we guess a tor cell is 600 bytes.

                if socket.inet_ntoa(ip.src) == self._src_ip:
                    if len(buf) % 600 == 0:
    ...
    def getCellIAT(self):
        ...
        return np.min(iat_list), np.max(iat_list), np.mean(iat_list), np.median(iat_list), np.std(iat_list), np.var(iat_list), np.quantile(iat_list, 0.25), np.quantile(iat_list, 0.75), skew(iat_list), \
               np.min(out_iat_list), np.max(out_iat_list), np.mean(out_iat_list), np.median(out_iat_list), np.std(out_iat_list), np.var(out_iat_list), np.quantile(out_iat_list, 0.25), np.quantile(out_iat_list, 0.75), skew(out_iat_list), \
               np.min(in_iat_list), np.max(in_iat_list), np.mean(in_iat_list), np.median(in_iat_list), np.std(in_iat_list), np.var(in_iat_list), np.quantile(in_iat_list, 0.25), np.quantile(in_iat_list, 0.75), skew(in_iat_list)
    ...
    def getCellBurst(self):
        ...
        return np.max(ori_burst), np.mean(ori_burst), np.median(ori_burst), np.std(ori_burst), np.var(ori_burst), np.quantile(ori_burst, 0.25), np.quantile(ori_burst, 0.75), skew(ori_burst), \
               np.max(out_burst), np.mean(out_burst), np.median(out_burst), np.std(out_burst), np.var(out_burst), np.quantile(out_burst, 0.25), np.quantile(out_burst, 0.75), skew(out_burst), \
               np.max(in_burst), np.mean(in_burst), np.median(in_burst), np.std(in_burst), np.var(in_burst), np.quantile(in_burst, 0.25), np.quantile(in_burst, 0.75), skew(in_burst),
    ...
    def get30CellBurst(self):
        ...
        return np.max(ori_burst), np.mean(ori_burst), np.median(ori_burst), np.std(ori_burst), np.var(ori_burst), np.quantile(ori_burst, 0.25), np.quantile(ori_burst, 0.75), skew(ori_burst), \
               np.min(out_burst), np.max(out_burst), np.mean(out_burst), np.median(out_burst), np.std(out_burst), np.var(out_burst), np.quantile(out_burst, 0.25), np.quantile(out_burst, 0.75), skew(out_burst), \
               np.min(in_burst), np.max(in_burst), np.mean(in_burst), np.median(in_burst), np.std(in_burst), np.var(in_burst), np.quantile(in_burst, 0.25), np.quantile(in_burst, 0.75), skew(in_burst)
    ...
    def getCellOrdering(self):
        ...
        return np.max(temp2), np.mean(temp2), np.median(temp2), np.std(temp2), np.var(temp2), np.quantile(temp2, 0.25), np.quantile(temp2, 0.75), skew(temp2), \
               np.max(temp1), np.mean(temp1), np.median(temp1), np.std(temp1), np.var(temp1), np.quantile(temp1, 0.25), np.quantile(temp1, 0.75), skew(temp1)
    ...
    def getCellConcentration(self):
        ...
        return np.max(out_concentration), np.mean(out_concentration), np.median(out_concentration), np.std(out_concentration), np.var(out_concentration), np.quantile(out_concentration, 0.25), np.quantile(out_concentration, 0.75), skew(out_concentration)
    ...
    def getFeature(self):
        ...
        max_IB, mean_IB, median_IB, std_IB, var_IB, fquan_IB, lquan_IB, skew_IB = self.getCellBurst()
        ...
        max_O_conc, mean_O_conc, median_O_conc, std_O_conc, var_O_conc, fquan_O_conc, lauqn_O_conc, skew_O_conc = self.getCellConcentration()

        f30In, f30Out, l30In, l30Out, percf30In, percf30Out, percl30In, percl30Out = self.getFirstLast30Cell()

        ntp, nop, nip, dura, pps, opps, ipps, bps, obps, ibps = self.getGeneralInformation()

        feature_array = [
            mean_C, std_C, var_C, skew_C, # 0-3
            ...
            f30_min_OB, f30_max_OB, f30_mean_OB, f30_median_OB, f30_std_OB, f30_var_OB, f30_fquan_OB, f30_lquan_OB, f30_skew_OB, # 63-71
            f30_min_IB, f30_max_IB, f30_mean_IB, f30_median_IB, f30_std_IB, f30_var_IB, f30_fquan_IB, f30_lquan_IB, f30_skew_IB, # 72-80
            ...
            max_O_conc, mean_O_conc, median_O_conc, std_O_conc, var_O_conc, fquan_O_conc, lauqn_O_conc, skew_O_conc, # 99-106
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--file_path', type=str, help='pcap file path', required=True)
    parser.add_argument('-s', 'src_ip', type=str, help='outgoing source ip address, i.e., 192.168.0.55', required=True)
    args = parser.parse_args()

    pcap_path = args.file_path
    src_ip = args.src_ip

    flow_generator = flowGenerator(pcap_path=pcap_path, ip=src_ip)
    traffic_feature = flow_generator.getFeature()
